...
public class Lognormal implements Distribution {
	...
	public Double mttf(Scanner scanner) {
		...
		try {
			...
			do {
				...
				if( sig <= 0 ) {
					System.out.println( "O valor do desvio padrão deve ser positivo e maior que zero" );
				}
			} while( sig <= 0 );
			
			/* mi + sig^2 / 2 */
			double aux = mi + ( ( sig * sig ) / 2 );
			
			double result = Math.pow( NEPER, aux );
	...
	public Double failureRate(Scanner scanner) {
		try {
			...
			/** f(t) **/
			double first = 1 / ( t * sig * Math.sqrt( 2 * 3.1415 ) );
			double auxExp = ((-1)*(1/2)) * Math.pow(( Math.log(t) - mi)/sig, 2);
	...
	public Double cumulativeFailureRate(Scanner scanner) {
		try {
			...
			while( it <= t ) {
				/** f(t) **/
				double first = 1 / ( it * sig * Math.sqrt( 2 * 3.1415 ) );
				double auxExp = ((-1)*(1/2)) * Math.pow(( Math.log(it) - mi)/sig, 2);
				
				Double ft = first * Math.pow( NEPER, auxExp );
				
				/** R(t) **/
				double z = ( Math.log( it ) - mi) / sig;
				double absZ = Math.abs( z );
